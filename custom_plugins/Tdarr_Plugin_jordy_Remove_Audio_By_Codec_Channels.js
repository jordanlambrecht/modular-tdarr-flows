...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  const codecsToRemove = (inputs.codecs || "")
    .toLowerCase()
    .split(",")
  ...
  if (codecsToRemove.length === 0) {
    response.infoLog +=
      "⚠️ No codecs specified for removal. Plugin will not process the file.\n"
    response.processFile = false
    return response
  }
  ...
  const languagesToFilter = (inputs.languages || "")
    .toLowerCase()
    .split(",")
    .map((lang) => lang.trim())
    .filter((lang) => lang !== "")
  ...
  response.infoLog += `\n🔵  Parameters: Codecs=${codecsToRemove.join(
    ","
  )}, Channels=${channelFilter}, Languages=${
    languagesToFilter.length > 0 ? languagesToFilter.join(",") : "any"
  }\n`
  ...
  for (let i = 0; i < file.ffProbeData.streams.length; i++) {
    ...
    if (stream.codec_type && stream.codec_type.toLowerCase() === "audio") {
      ...
      // Debug info
      response.infoLog += `🔵 Audio track ${audioIdx}: codec=${currentCodec}, channels=${currentChannels}, language=${
      ...
      const channelMatches =
        targetChannelCount === null || currentChannels === targetChannelCount
