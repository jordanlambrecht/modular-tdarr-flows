...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  if (audioStreams.length === 0) {
    response.infoLog +=
      "No audio streams found. Breaking out of plugin stack.\n"
    return response
  }

  // Parse inputs
  const codecsToProcess = inputs.codecsToProcess
    ? inputs.codecsToProcess
        .toLowerCase()
        .split(",")
        .map((item) => item.trim())
    : []
  const channelsToProcess = inputs.channelsToProcess
    ? inputs.channelsToProcess
        .split(",")
        .map((item) => parseInt(item.trim(), 10))
    : []
  const codecsToNotProcess = inputs.codecsToNotProcess
    ? inputs.codecsToNotProcess
        .toLowerCase()
        .split(",")
        .map((item) => item.trim())
    : []
  const channelsToNotProcess = inputs.channelsToNotProcess
    ? inputs.channelsToNotProcess
        .split(",")
        .map((item) => parseInt(item.trim(), 10))
    : []
  const requireAllStreams =
    inputs.requireAllStreams === "true" || inputs.requireAllStreams === true
  ...
  const streamMatchesCriteria = (stream) => {
    ...
    if (
      codecsToNotProcess.length > 0 &&
      codecsToNotProcess.includes(codecName)
    ) {
      return false
    }
    ...
    if (
      channelsToNotProcess.length > 0 &&
      channelsToNotProcess.includes(channels)
    ) {
      return false
    }
  ...
  if (shouldProcess) {
    response.infoLog +=
      "File meets audio codec and channel criteria. Moving to next plugin.\n"
    ...
    response.infoLog +=
      "File does not meet audio codec and channel criteria. Breaking out of plugin stack.\n"
